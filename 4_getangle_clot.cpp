...
public:
  ...
  void callback(const egolane::datax& input)
  {
	...
	ROS_INFO("I heard ROAD PARAMETERS: [%f, %f, %f]", point.x1, point.x2, point.x3);
	  ...
	  double C3 = point.x3;
		...
		//m=f'(x4)			cn = - m*x4 + y4
		  ...
		  double m = C1 + 2*C2*x4 + 3*C3*x4*x4;
		  double cn = -m*x4 + y4;
		  ...
		  if (-m<0){ 
			...
			pub_.publish(angled);
				...
				ofstream myfileAngle;
  				myfileAngle.open ("00angle.txt");
					if (myfileAngle.is_open())
					{
					   myfileAngle<<angle;
					   myfileAngle.close();
