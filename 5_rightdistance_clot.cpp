...
public:
  SubscribeAndPublish()
  {
    ...
    pub_ = n_.advertise<egolane::datax>("RightDistance", 1);
  ...
  void callback(const egolane::datax& input)
  {
	...
	ROS_INFO("I heard ROAD RIGHT Y POINTS: [%f, %f, %f, %f]", point.y0, point.y1, point.y2, point.y3);
  	ROS_INFO("I heard ROAD RIGHT X POINTS: [%f, %f, %f, %f]", point.x0r, point.x1r, point.x2r, point.x3r);
  
	  
   	  double alpha = point.x0;	  
	  double xt = 320;

	  double cx0 = point.x0r;
	  double cx1 = point.x1r;
	  double cx2 = point.x2r;
	  double cx3 = point.x3r;
...
ofstream myfile0 ("0RoadRightPoints0.txt", std::ios::app);

	ofstream myfile1 ("1RoadRightPoints1.txt", std::ios::app);
	ofstream myfile2 ("2RoadRightPoints2.txt", std::ios::app);
	ofstream myfileOK ("3RoadRightPointsOK.txt", std::ios::app);
	...
	if (z==zz){
		...
		double dif00 = abs (point.x0r - xp0[z0]);
		double dif01 = abs (point.x1r - xp0[z1]);
		double dif02 = abs (point.x2r - xp0[z2]);
		double dif03 = abs (point.x3r - xp0[z3]);
		double dif0 = dif00 + dif01 + dif02 + dif03;


		double dif10 = abs (point.x0r - xp1[z0]);
		double dif11 = abs (point.x1r - xp1[z1]);
		double dif12 = abs (point.x2r - xp1[z2]);
		double dif13 = abs (point.x3r - xp1[z3]);
		double dif1 = dif10 + dif11 + dif12 + dif13;


		double dif20 = abs (point.x0r - xp2[z0]);
		double dif21 = abs (point.x1r - xp2[z1]);
		double dif22 = abs (point.x2r - xp2[z2]);
		double dif23 = abs (point.x3r - xp2[z3]);
		...
		if ( dif0<=dif1 && dif0<=dif2){
			...
			for (int y=0;y<z; y++){
				...
				cout<<"Y = "<<z-1+y00<<endl;
				cout<<"Right Distance = "<<ca<<endl<<endl<<endl;
...
{
  //Initiate ROS
  ros::init(argc, argv, "rightdistance");
